...
int julia(t_fractal *fractal)
{
    ...
    z.re = (fractal->x / fractal->zoom) - (WIDTH / (2.0 * fractal->zoom)) + fractal->offset_x;
    z.im = (fractal->y / fractal->zoom) - (HEIGHT / (2.0 * fractal->zoom)) + fractal->offset_y;
    c.re = 0.41031;
    c.im = -0.67037;
...
int main(void)
{
    ...
    fractal.win = mlx_new_window(fractal.mlx, WIDTH, HEIGHT, "Julia Set");
